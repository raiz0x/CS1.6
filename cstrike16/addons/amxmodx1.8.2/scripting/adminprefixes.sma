...
// cand schimba numele, de facut sa arate si tagu..si daca mai are tag >>
...
new str_id[200], temp_key[200], temp_prefix[200], temp_value;
new bool:g_toggle[33]; // = true ?
...
public HookSay(id)
{
	...
	if(temp_prefix[0])	formatex(g_message, charsmax(g_message), "^1%s^4%s^3 %s^1 :^4 %s", say_team_info[is_user_alive(id)][g_team], temp_prefix, g_name, g_typed)

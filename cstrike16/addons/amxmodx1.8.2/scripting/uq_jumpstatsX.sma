...
public Log_script(f_frames,cheated_frames,id,Float:log_dist,Float:log_max,Float:log_pre,log_str,log_sync,jump_type_str[],wpn_str[],punishments[],t_str[40*NSTRAFES])
{
	...
	ColorChat( id, NORMAL, "!t[!g %s!t ]!g %s!y a sarit:!t [!g %.03f!t ]!y.  !t [!g %d!t ]!y Strafe &!t [!g %d%!t ] Sync", jump_type_str, username, log_str, log_sync );
	...
	if(uq_script_notify)
	{
		ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_SCRIPTNOTIFY",prefix,username,jump_type_str);
		ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_SCRIPTNOTIFY",prefix,username,jump_type_str);
		ColorChat(id,GREY,"%L",LANG_SERVER,"UQSTATS_SCRIPTNOTIFY",prefix,username,jump_type_str);
	}
}
public krasnota(id)
{                
							...
							if( !failed_jump[id] )
							{
										     ...
										     || jump_type[id]==Type_ladderBhop || jump_type[id]==Type_ladder || jump_type[id]==Type_LongJump || jump_type[id]==Type_HighJump))
								{
									if ( distance[id] >= god_dist ) {
										...
										ColorChat(ids, RED, "%L",LANG_SERVER,"UQSTATS_CCHAT_NORMALA",prefix, g_playername[id], distance[id],Jtype[id],block_str,weapon_name,pre_type[id],airacel[id]);
									}
										...
										ColorChat(ids, BLUE, "%L",LANG_SERVER,"UQSTATS_CCHAT_NORMALA",prefix, g_playername[id], distance[id],Jtype[id],block_str,weapon_name,pre_type[id],airacel[id]);
									}
									else if ( distance[id] >= pro_dist ) {
										if( uq_sounds && enable_sound[id]==true ) client_cmd(id, "speak misc/perfect");
									
										ColorChat(ids, GREEN, "%L",LANG_SERVER,"UQSTATS_CCHAT_NORMALB",prefix, g_playername[id], distance[id],Jtype[id],block_str,weapon_name,pre_type[id],airacel[id]);
									}
									else if ( distance[id] >=good_dist ) {
										if( uq_sounds && enable_sound[id]==true ) client_cmd(id, "speak misc/impressive");
									
										ColorChat(ids, GREY, "%L",LANG_SERVER,"UQSTATS_CCHAT_NORMALA",prefix, g_playername[id], distance[id],Jtype[id],block_str,weapon_name,pre_type[id],airacel[id]);
									}
								}
								else if(jump_type[id]==Type_Multi_CountJump || (multiscj[id]==2 && jump_type[id]==Type_StandUp_CountJump) || (multidropcj[id]==2 && jump_type[id]==Type_Drop_CountJump))
								{
									if ( distance[id] >= god_dist ) {
										...
										if( uq_light ) krasnota(id);
										ColorChat(ids,RED,"%L",LANG_SERVER,"UQSTATS_COLORCHAT_MULTIDUCKA",prefix, g_playername[id], distance[id],Jtype[id],ducks[id],block_str,weapon_name,pre_type[id],airacel[id]);
									}
									else if ( distance[id] >= leet_dist  ) {
										if( uq_sounds && enable_sound[id]==true ) client_cmd(id, "speak misc/mod_wickedsick");
										...
										if( uq_light ) sineva(id);
										ColorChat(ids,BLUE,"%L",LANG_SERVER,"UQSTATS_COLORCHAT_MULTIDUCKA",prefix, g_playername[id], distance[id],Jtype[id],ducks[id],block_str,weapon_name,pre_type[id],airacel[id]);
									}
									else if ( distance[id] >= pro_dist ) {
										if( uq_sounds && enable_sound[id]==true ) client_cmd(id, "speak misc/perfect");
									
										ColorChat(ids,GREEN,"%L",LANG_SERVER,"UQSTATS_COLORCHAT_MULTIDUCKB",prefix, g_playername[id], distance[id],Jtype[id],ducks[id],block_str,weapon_name,pre_type[id],airacel[id]);
									}
									else if ( distance[id] >=good_dist ) {
										if( uq_sounds && enable_sound[id]==true ) client_cmd(id, "speak misc/impressive");
									
										ColorChat(ids,GREY,"%L",LANG_SERVER,"UQSTATS_COLORCHAT_MULTIDUCKA",prefix, g_playername[id], distance[id],Jtype[id],ducks[id],block_str,weapon_name,pre_type[id],airacel[id]);
									}
								}
							}	
						}
...
{	
	if( !gHasColorChat[id] )
	{
		...
		ColorChat(id,GREEN,"%L",LANG_SERVER,"UQSTATS_COLORCHAT_E", prefix);
	}
	else 
	{
		gHasColorChat[id] = false;
		
		ColorChat(id,GREEN,"%L",LANG_SERVER,"UQSTATS_COLORCHAT_D", prefix);
	}
...
public cmdljStats( id ) {
	...
	{
		ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_LJSTATS_D", prefix);
		g_lj_stats[id]=false;
	...
	else 
	{
		...
		g_lj_stats[id]=true;
		ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_LJSTATS_E", prefix);
	}
...
{		
	ColorChat(id,GREY,"%L",LANG_SERVER,"UQSTATS_VERSION_1", prefix,VERSION);
	
	if(kz_sql==1)
	{
		ColorChat(id,GREY,"%L",LANG_SERVER,"UQSTATS_VERSION_SQL1", prefix);
		ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_VERSION_SQL2", prefix);
	}
	else ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_VERSION_2", prefix);
}
public pre_stats(id)
{	
	if(kz_stats_pre[id]==true) 
	{
		ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_PRESTATS_D", prefix);
		kz_stats_pre[id]=false;
	}
	else 
	{
		kz_stats_pre[id]=true;
		ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_PRESTATS_E", prefix);
	}
}
...
{	
	...
	{
		ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_STRAFESTATS_D", prefix);
		streifstat[id]=false;
	}
	else 
	{
		streifstat[id]=true;
		ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_STRAFESTATS_E", prefix);
	}
}
public cmdljbeam(id)
{
	if(ingame_strafe[id])
	{
		ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_BEAMDONTWORK", prefix);
	}
	else
	{
		if(kz_beam[id]==true) 
		{
			ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_BEAMSTATS_D", prefix);
			kz_beam[id]=false;
		}
		else 
		{
			kz_beam[id]=true;
			ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_BEAMSTATS_E", prefix);
		}
	}
}
public show_pre(id)
{
	if(showpre[id]==true) 
	{
		ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_SHOWPRE_D", prefix);
		showpre[id]=false;
	}
	else 
	{
		showpre[id]=true;
		ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_SHOWPRE_E", prefix);
	}
}
public show_speed(id)
{
	...
	if(uq_admins==1 && !player_admin[id])
	{
		ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_SPEED_D1", prefix);
	}
	else if(g_team!=uq_team && !uq_speed_allteam)
	{
		ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_SPEED_D2", prefix);
	}
	else
	{
		if(jofon[id])
		{
			ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_SPEED_D3", prefix);
		}
		else
		{
			if(speedon[id]==false) 
			{
				ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_SPEED_E", prefix);
				speedon[id]=true;
			...
			else 
			{
				...
				ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_SPEED_D", prefix);
			}
		}
	}
}
public trainer_jof(id)
{
	...
	if(uq_admins==1 && !player_admin[id])
	{
		ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_JOFTR_D1", prefix);
	}
	else if(g_team!=uq_team)
	{
		ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_JOFTR_D2", prefix);
	}
	else
	{
		if(speedon[id])
		{
			ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_JOFTR_D3", prefix);
		}
		else
		{
			if(jofon[id]==false) 
			{
				ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_JOFTR_E", prefix);
				jofon[id]=true;
				jof[id]=0.0;
			...
			else 
			{
				...
				ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_JOFTR_D", prefix);
			}
		}
	}
}
public speed_type(id)
{
	...
	{
		ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_SPEED_B", prefix);
		speedtype[id]=true;
	}
	else 
	{
		speedtype[id]=false;
		ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_SPEED_S", prefix);
	}
}
public show_jheight(id)
{
	...
	{
		ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_JHEIGH_E", prefix);
		jheight_show[id]=true;
	}
	else 
	{
		jheight_show[id]=false;
		ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_JHEIGH_D", prefix);
	}
}
public show_jof(id)
{
	...
	{
		ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_JOF_E", prefix);
		showjofon[id]=true;
	}
	else 
	{
		showjofon[id]=false;
		ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_JOF_D", prefix);
	}
}
public show_early(id)
{
	...
	{
		ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_BHOPWARN_E", prefix);
		failearly[id]=true;
	}
	else 
	{
		failearly[id]=false;
		ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_BHOPWARN_D", prefix);
	}
}
public multi_bhop(id)
{
	...
	{
		ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_MULTIBHOP_E", prefix);
		multibhoppre[id]=true;
	}
	else 
	{
		multibhoppre[id]=false;
		ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_MULTIBHOP_D", prefix);
	}
}
public duck_show(id)
{
	...
	{
		ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_DUCKSPRE_E", prefix);
		showduck[id]=true;
	}
	else 
	{
		showduck[id]=false;
		ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_DUCKSPRE_D", prefix);
	}
}
public lj_show(id)
{
	if(ljpre[id]==false) 
	{
		ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_LJPRE_E", prefix);
		ljpre[id]=true;
	}
	else 
	{
		ljpre[id]=false;
		ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_LJPRE_D", prefix);
	}
}
public enable_sounds(id)
{
	if(uq_sounds)
	{
		...
		{
			ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_SOUND_E", prefix);
			enable_sound[id]=true;
		}
		else 
		{
			enable_sound[id]=false;
			ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_SOUND_D", prefix);
		}
	}
	else ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_SOUND_D1", prefix);
}
public ShowedgeFail(id)
{
	if(Show_edge_Fail[id]==false) 
	{
		ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_EDGEF_E", prefix);
		Show_edge_Fail[id]=true;
	}
	else 
	{
		Show_edge_Fail[id]=false;
		ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_EDGEF_D", prefix);
	}
}
public Showedge(id)
{
	...
	{
		ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_EDGE_E", prefix);
		Show_edge[id]=true;
	}
	else 
	{
		Show_edge[id]=false;
		ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_EDGE_D", prefix);
	}
}
public heightshow(id)
{
	...
	{
		ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_FALLH_E", prefix);
		height_show[id]=true;
	}
	else 
	{
		height_show[id]=false;
		ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_FALLH_D", prefix);
	}
}
public ingame_st_stats(id)
{
	if(uq_istrafe)
	{
		if(kz_beam[id])
		{
			ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_ISTRAFE_D1", prefix);
		}
		else
		{
			if(ingame_strafe[id]==false) 
			{
				ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_ISTRAFE_E", prefix);
				ingame_strafe[id]=true;
			}
			else 
			{
				ingame_strafe[id]=false;
				ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_ISTRAFE_D", prefix);
			}
		}
	}
	else ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_ISTRAFE_D2", prefix);
...
public ResetHUD(id)
{
	if(is_user_alive(id) && !is_user_bot(id) && !is_user_hltv(id) )
	{
		if(firstshow[id]==false)
		{
			...
			{
				ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_AIRACCEL100", prefix);
			}
			else if( uq_airaccel==0 || uq_airaccel==10)
			{
				ColorChat(id,BLUE,"%L",LANG_SERVER,"UQSTATS_AIRACCEL10", prefix);
			}
			else 
				ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_AIRACCEL", prefix,uq_airaccel);*/
				
			firstshow[id]=true;
			
			if(equali(mapname,"slide_gs_longjumps") || equali(mapname,"b2j_slide_longjumps"))
			{
				ColorChat(id,RED,"%L",LANG_SERVER,"UQSTATS_SLIDE", prefix);
			}
		}

...
public print_adminlist(user) 
{
    ...
    if(count > 0) 
    {
        for(x = 0 ; x < count ; x++) 
        {
            len += format(message[len], 255-len, "^x01[^x04 %s^x01 ]^x01 %s ", adminnames[x], x < (count-1) ? " | ":"");
            //len += format(message[len], 255-len, "^x03[HnS.Ecila.Ro-ViPs]^x01 [^x04 %s^x01 ]^x01 %s ", adminnames[x], x < (count-1) ? " | ":"");
            if(len > 96) 
            {

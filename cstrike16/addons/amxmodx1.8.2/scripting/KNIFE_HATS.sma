...
public Set_Hat(player, imodelnum)
{
	...
	if(setting[player]==0)	if(g_HatEnt[player] > 0)	fm_set_entity_visibility(g_HatEnt[player],0)
	else
	{
...
else if( !PlayerHasTag[ id ] )
{
		switch( cs_get_user_team( id ) )
		{
			case CS_TEAM_T:		ColorChat( 0, RED,"^1%s [Level:^4 %d^1]^3 %s^1: %s", is_user_alive( id ) ? "" : "*DEAD* ",szName, szChat );
			case CS_TEAM_CT:	ColorChat( 0, BLUE,"^1%s [Level:^4 %d^1]^3 %s^1: %s", is_user_alive( id ) ? "" : "*DEAD* ",szName, szChat );
			case CS_TEAM_SPECTATOR:	ColorChat( 0, GREY,"^1*SPEC* [Level:^4 %d^1]^3 %s^1: %s",szName, szChat );
		}
}

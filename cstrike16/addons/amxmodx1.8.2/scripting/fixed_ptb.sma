...
new PTB_LIMITMIN = 3 // number of minimum players on map for team limiting
...
new PTB_WTJAUTO = 0 // wtj tries needed to become autojoined
new PTB_WTJKICK = 0 // wtj tries needed to become kicked
new bool:PTB_KICK = false // kick for wtj counts
...
new PTB_FORCESWITCH = 5 // number of tries after which PTB switches alive, if neccessary
new bool:PTB_DEADONLY = true // switch dead only

// messages
new bool:PTB_TELLWTJ = false // tell about wtj tries
new bool:PTB_ANNOUNCE = false // announce team status at beginning of round
new bool:PTB_SAYOK = false // announce team status, if teams are alright
new bool:PTB_TYPESAY = true // use typesay

// team strength limits
new PTB_MAXSTREAK = 3 // max. allowed team win streak
new PTB_MAXSCORE = 3 // max. allowed team score difference
...
public plugin_init(){
	...
	limitjoin_immunity = register_cvar("ptb_limitjoin_immunity", "0")
	...
	register_clcmd("say_team /last","check_lasttransfer")
#endif
...
transferPlayer(id){
	...
	if(cs_get_user_defuse(id))	cs_set_user_defuse(id, 0);
...
actAtEndOfRound(){
	...
	if (winnerTeam) {
		...
		if(get_pcvar_num(transfer_type) == 3){
			...
			if ((teamCounts[winnerTeam]+(PTB_MAXDIFF/2)) < teamCounts[loserTeam])	doSwitch()	
			else if (teamCounts[loserTeam] <= (teamCounts[winnerTeam]+(PTB_MAXDIFF/2)))	doTransfer()
		}
	}
}
...
doSwitch() {
	...
	if(cs_get_user_team(winner)==CS_TEAM_SPECTATOR||cs_get_user_team(loser)==CS_TEAM_SPECTATOR)	return
...
doTransfer() {
	...
	get_user_name(winner,winnerName,31)
	transferPlayer(winner)
	if(cs_get_user_team(winner)==CS_TEAM_SPECTATOR)	return
...
checkTeamBalance() {
	...
	if (teamScores[TS] - teamScores[CTS] > PTB_MAXSCORE && tRating >= PTB_MINRATING)	wtConditions[TS]++
	...
	if (winStreaks[TS] > PTB_MAXSTREAK && tRating >= PTB_MINRATING)	wtConditions[TS]++

	if (winStreaks[CTS] > PTB_MAXSTREAK && ctRating >= PTB_MINRATING)	wtConditions[CTS]++

	// check ratings
	if (tRating >= PTB_MAXRATING)	wtConditions[TS]++

	if (ctRating >= PTB_MAXRATING)	wtConditions[CTS]++

	// check ratings
	if (tRating >= PTB_SUPERRATING)	wtConditions[TS]++

	if (ctRating >= PTB_SUPERRATING)	wtConditions[CTS]++

	// check team sizes for unequal ratings
	if (teamCounts[TS] > teamCounts[CTS] && tRating >= PTB_MINRATING)	wtConditions[TS]++
...
public round_end(){
	...
	else	return // %!MRAD_rounddraw (both teams have left the game)
...
public client_disconnect(id) {
	...
	new a = get_maxplayers()
	for (new i = 1; i <= a; ++i)	++teamCounts[playerTeam[i]]

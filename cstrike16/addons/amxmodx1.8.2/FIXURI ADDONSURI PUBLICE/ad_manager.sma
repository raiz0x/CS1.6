...
new const Float:RAND_MIN = 40.0;

//Maximum difference between two different ads (float)
new const Float:RAND_MAX = 45.0;
...
public load()
{
	...
	{
		new output[512], conditions[138], temp[64], type;
		...
		for(new a = 0; a < MAXADS && !feof(fHandle); a++)
		{
			...
			if(output[0] == COND_TKN)
			{
				...
				split(output, conditions, charsmax(conditions), output, 511, DEVIDE_STKN);
				...
				for(new b = 0; b < 3; b++)
				{
					...
					split(conditions, temp, 64, conditions, charsmax(conditions), COND_STKN);
			...
			if(type == SAY_AD)
			{
				new said[32], answer[138];
				
				//Remove the @ from line
				output[0] = ' ';
				trim(output);
				
				split(output, said, 31, answer, charsmax(answer), DEVIDE_STKN);
				
				//Apply color
				setColor(answer, charsmax(answer));
...
public displayAd(params[])
{
	...
	new message[135];
	getString(STORE, params[1], message, charsmax(message), params[0], params[1]);
